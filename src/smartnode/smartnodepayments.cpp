...
*
*   Why is this needed?
*   - In smartcoin some blocks are superblocks, which output much higher amounts of coins
...
{
    if(fLiteMode) return; // disable all smartcoin specific functionality
...
bool CSmartnodeBlockPayees::IsTransactionValid(const CTransaction& txNew)
{
    ...
    LogPrintf("CSmartnodeBlockPayees::IsTransactionValid -- ERROR: Missing required payment, possible payees: '%s', amount: %f smartcoin\n", strPayeesPossible, (float)nSmartnodePayment/COIN);
    return false;
}
